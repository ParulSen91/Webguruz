...
if ( ! function_exists( 'presscore_get_page_title' ) ) :
	...
	function presscore_get_page_title() {
		...
		if ( is_page() || is_single() ) {
			...
			if ( is_category() ) {
				$title = sprintf( __( 'Category : %s', 'the7mk2' ), '<span>' . single_cat_title( '', false ) . '</span>' );
				...
				the_post();
				$title = sprintf( __( 'Author : %s', 'the7mk2' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
				rewind_posts();
