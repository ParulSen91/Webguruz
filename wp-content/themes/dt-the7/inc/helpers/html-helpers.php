...
if ( ! function_exists( 'presscore_get_post_data' ) ) :
	...
	function presscore_get_post_data( $html = '' ) {
		...
		$html .= sprintf(
			'<a title="%s" class="data-link" rel="bookmark"><time class="entry-date updated" datetime="%s">%s</time></a>',
	...
	function presscore_post_details_link( $post_id = null, $class = array('details', 'more-link'), $link_text = null ) {
		...
		if ( $url ) {
			$output = sprintf(
				...
				is_string( $link_text ) ? $link_text : __( 'Read more', 'the7mk2' )
			);
		}
		...
		foreach ( $buttons as $button ) {
			...
			switch( $button ) {
				...
				case 'pinterest':
					...
					$url = 'https://www.pinterest.com/pin/create/button/';
					$custom = ' data-pin-config="above" data-pin-custom="true" data-pin-do="buttonBookmark"';
					// if image
	...
	function presscore_display_post_author() {
		...
		<div class="entry-author wf-table">
			...
			<div class="wf-td entry-author-info">
                            <?php echo  $avatar = get_avatar( get_the_author_meta('user_email') , apply_filters( 'twentyeleven_author_bio_avatar_size', 60 ) ); ?>
...
if ( ! function_exists( 'presscore_get_post_author' ) ) :
	...
	function presscore_get_post_author( $html = '' ) {
		...
		$html .= sprintf(
			'<a class="author vcard" href="%s" title="%s" rel="author">%s %s</a>',
				esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				esc_attr( sprintf( __( 'View all posts by %s', 'the7mk2' ), get_the_author() ) ),
                                get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'twentyeleven_author_bio_avatar_size', 60 ) ),
				sprintf( __( 'By %s', 'the7mk2' ), '<span class="fn">' . $author[0] . '</span>' )
		);
