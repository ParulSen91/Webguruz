...
?>
	...
	$comment_form_args = array(
		...
		'comment_field'	=> '<span><p class="comment-form-comment"><label class="assistive-text" for="comment">' . __( 'Comment *', 'the7mk2' ) . '</label><textarea  maxlength="200" id="comment" placeholder="' . __( 'Comment *', 'the7mk2' ) . '" class="condes" name="comment" cols="45" rows="8" aria-required="true"></textarea></p></span><p class="charNum"></p>',
    ...
    commentform.submit(function(){
      ...
      $.ajax({
        ...
        error: function(data){
        	...
        	var strng=data.responseText
    
        console.log(strng)
   var arr = strng.split('<p>');
   $('#comment-status').html($.trim(arr[1])).css('color','red')
        },
        success: function(data, textStatus){
        	
        
   var myString=$(data).text();
   var arr = myString.split('html');
			   if($.trim(arr[0])=='Comment Submission Failure'){
			   		$('#comment-status').html($.trim(arr[0])).css('color','red')

			   }
			   else if(data !=''){
			   	var arr = data.split('<p>');
  				 $('#comment-status').html('Comment sent successfully').css('color','green')
