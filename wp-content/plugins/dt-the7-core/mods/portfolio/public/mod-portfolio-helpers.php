...
if ( ! function_exists( 'presscore_project_get_preview_content' ) ) :
	...
	function presscore_project_get_preview_content() {
		...
		if ( $config->get( 'show_titles' ) && $title = get_the_title() ) {
			$html .= sprintf( '<h3 class="entry-title"><a href="%s" title="%s" rel="bookmark">%s</a></h3>', '#', the_title_attribute( 'echo=0' ), $title );
		}

		// description
		if ( $config->get( 'show_excerpts' ) ) {
			$html .= substr(get_the_excerpt(), 0, 300).'...'; //apply_filters( 'the_excerpt',  );
		

                // 5-sept-2016, add tags custom code -jai
                $terms = get_the_terms($post->ID, 'tag');
                if(isset($terms))
                {
                $html .= '<h3 class="purtsub">Technology Used: </h3>';
                $html .= '<ul>';
                ...
                $html .= $project_link = presscore_get_project_link( 'project-link' );
                $html .= '</div>';
                }
                 }
		// details button
