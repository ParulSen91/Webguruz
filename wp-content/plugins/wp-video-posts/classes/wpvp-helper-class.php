...
class WPVP_Helper{
	...
	*@access static
	**/
	static function wpvp_clint_categories_dropdown($echo=true,$selected=0){
		$html = '';
		
		$args = array('taxonomy'=>'ct_testimonials');
		$categories = get_categories($args);
		if(is_array($categories)&&!empty($categories)):
			$html = '<select name="wpvp_category" class="wpvp_select wpvp_require">';
			foreach($categories as $category){
				$html .= '<option ';
				$html .= ' value="'.$category->term_id.'" '.selected($selected,$category->term_id,false).'>';
				$html .= $category->cat_name.'</option>';
