	...
	public function wpvp_widget_latest_posts($instance){
		...
		while($vid_posts->have_posts()):
			...
			$video_fp_meta_array = get_post_meta($postID, 'wpvp_fp_code',false);
                        $src_v = get_post_meta($postID, 'video_url',true);
			if(!empty($video_meta_array)||!empty($video_fp_meta_array)){
				...
				if(($display_type=='th')||($display_type=='')){
					...
					if(is_numeric($video_meta)){
						...
						if($wpvp_player=='flowplayer'){
							$flowplayer_code = '<a href="'.$src_v.'" class="myPlayer" style="display:block;width:'.$width.'px;height:'.$height.'px;margin:10px auto"><img width="'.$width.'" height="'.$height.'" src="'.$splash.'" alt="" /></a>';
				...
				if($post_title!=''){
        			        $video_item .= '<div class="wpvp_video_title">'.get_the_title().'</div>';
	        		}
                                if($position!=''){
					$video_item .= '<span class="wpvp_position">'.get_post_meta($postID, 'wpvp_position',true).'</span>';
				}
                                if($company!=''){
					$video_item .= '<span class="wpvp_company">'.get_post_meta($postID, 'wpvp_company',true).'</span>';
				}
				if($author!=''){
